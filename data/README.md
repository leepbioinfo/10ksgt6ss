...
- `control.xlsx`: Excel table listing domain annotations and T6SS components. This data is used by scripts `fdevolvedcargo1.py` and `fdevolvedcargo2.py`.
- `coord.pkl`: table of coordinates for domains detected by HMMs.
- `domains.tsv`: table of 10KSG protein domain hits to HMM models from Pfam, CDD, TXSScan. 
- `FDEvolvedCargo5.xlsx`: Excel table listing protein IDs detected by HMM toxin models used in this study, including genomic site classification and fusions to T6SS components. This file was automatically generated by the scripts `fdevolvedcargo1.py` and `fdevolvedcargo2.py`
...
- `info.pkl`: table of matches to Pfam and other model sources.
...
- `models.xlsx`: HMM annotation table mapping each model to the protein domain function. This data is used by scripts `fdevolvedcargo1.py` and `fdevolvedcargo2.py`.
- `modex.tsv`: table of model from Pfam and other sources we choose to ignore while searching toxins (see fdevolvedcargo2.py).
- `rename_model_name.yaml`:  YAML file used to rename selected models during the final analysis stage.
- `repseqs.xlsx`: tabe of toxins with representative sequences.
